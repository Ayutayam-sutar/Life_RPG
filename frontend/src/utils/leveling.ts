import { QuestDifficulty, AttributeType } from '../types';
...
export function calculateLevelFromTotalXp(totalXp: number): {
  ...
  while (true) {
    ...
    if (remainingXp < needed) {
      return { level, currentLevelXp: remainingXp, nextLevelXp: needed, progressPercent: Math.min(100, Math.max(0, Math.round((remainingXp / needed) * 100))) };
    }
    remainingXp -= needed;
    level++;
  }
...
export function calculateQuestRewards(difficulty: QuestDifficulty, streakDays: number) {
  ...
  return { xp: Math.round(config.baseXp * multiplier), gold: Math.round(config.baseGold * multiplier), multiplier, attributePoints: config.attributePoints };
}

export function getHeroTitle(level: number, dominantAttribute?: AttributeType): string {
  if (level < 3) return 'Novice Achiever';
  const role = dominantAttribute === 'strength' ? 'Athlete' : dominantAttribute === 'intellect' ? 'Strategist' : dominantAttribute === 'charisma' ? 'Communicator' : 'Achiever';
